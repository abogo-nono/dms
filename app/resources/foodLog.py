...
class FoodLogResource(Resource):
    def post(self):
        ...
        allergy = Allergy.query.filter_by(
            person_id=args["person_id"], food_id=args["food_id"]
        ).first()

        # Create the food log
        food_log = FoodLog(
            person_id=args["person_id"], food_id=args["food_id"], notes=args["notes"]
        )

        db.session.add(food_log)
        db.session.commit()

        result = {"success": True, "food_log": food_log.to_dict()}

        # Add allergy warning if applicable
        if allergy:
            result["warning"] = (
                f"Warning: You have a {allergy.severity} allergy to {food.name}"
            )
...
class PersonFoodLogResource(Resource):
    def get(self, person_id):
        ...
        query = FoodLog.query.filter_by(person_id=person_id).order_by(
            FoodLog.timestamp.desc()
        )
