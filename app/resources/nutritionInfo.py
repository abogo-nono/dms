...
parser.add_argument(
    "calories", type=float, required=True, help="Calories cannot be blank"
)
parser.add_argument(
    "protein", type=float, required=True, help="Protein cannot be blank"
)
parser.add_argument("fat", type=float, required=True, help="Fat cannot be blank")
parser.add_argument(
    "carbohydrates", type=float, required=True, help="Carbohydrates cannot be blank"
)
...
class NutritionInfoListResource(Resource):
    ...
    def post(self):
        ...
        if NutritionInfo.query.filter_by(food_id=args["food_id"]).first():
            return {
                "message": "Nutrition information for this food already exists"
            }, 400
